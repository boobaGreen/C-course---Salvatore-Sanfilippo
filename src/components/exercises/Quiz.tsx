...
/**
 * Helper component to render text with basic Markdown-like formatting:
          ...
          {line.split(/(`[^`]+`)/g).map((part, partIdx) => {
            if (part.startsWith("`") && part.endsWith("`")) {
              return (
                <code
                  key={partIdx}
                  className="bg-black/30 text-[var(--color-brand-secondary)] px-1.5 py-0.5 rounded font-mono text-[0.9em] border border-white/5"
                >
                  {part.slice(1, -1)}
                </code>
              );
...
export default function Quiz({
  ...
  const handleConfirm = () => {
    ...
    if (selectedOption === questions[currentStep].correctAnswer) {
      ...
      addXP(
        50,
        `activity-quiz-${lessonSlug}-${quizTitle}-q${questions[currentStep].id}`,
      );
    }
  };
    ...
    <div className="my-8 glass-panel rounded-2xl border-white/10 overflow-hidden shadow-2xl shadow-black/20 text-zinc-300">
          ...
          {currentQuestion.options.map((option, index) => {
            ...
            if (isAnswered) {
              if (index === currentQuestion.correctAnswer) {
                ...
                stateClass =
                  "bg-white/5 border-white/10 text-zinc-500 opacity-50";
              }
            } else if (selectedOption === index) {
              stateClass =
                "bg-[var(--color-brand-secondary)]/20 border-[var(--color-brand-secondary)]/50 text-[var(--color-brand-secondary)]";
            }
      ...
      <div className="h-1 bg-white/5 w-full">
        <motion.div
          ...
          animate={{
            width: `${((currentStep + (isAnswered ? 1 : 0)) / questions.length) * 100}%`,
          }}
        />
      </div>
    </div>
